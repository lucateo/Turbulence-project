...
# Energy spectrum computation trying to do averages (basically this was a failure)
...
k_scal_bound = np.linspace(0,np.max(k_module_full),N)
intval = k_scal_bound[1] - k_scal_bound[0]
k_scal_mean = k_scal_bound[:-1]+1/2*intval
E = 0
n=0
E_mean = np.zeros_like(k_scal_mean)
def power(omega_hat,t):
	global E
	global n
	global E_mean
	...
	for i in range(len(k_scal_mean)):
		S_scal=1/2*np.sum(u_hat_sq[np.logical_and(k_scal_bound[i]<k_module_full,k_module_full<k_scal_bound[i+1])])
	E_mean = (n*E_mean +S_scal)/(n+1)
	n=n+1
	if(n%20==0):
		print(E_mean)
		print(E_mean[1])
	return [E_mean, k_scal_mean, E]
